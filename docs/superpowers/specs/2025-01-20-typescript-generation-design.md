...
Primitives map directly: strings to string, numbers to number, booleans to boolean, null to null. Arrays convert using the user's chosen syntax format—either shorthand or generic—and objects render as `{ ... }` inline or extracted as separate interface/type blocks depending on mode selection.

#### Algorithm

1. Parse JSON input
2. If `nestedMode === 'inline'`: recursively generate type literals inline
3. If `nestedMode === 'extracted'`: collect all nested objects with generated names, then generate root type referencing extracted types plus each extracted type separately
...
Recursively build a type string from the parsed JSON, starting with primitives returning their TS equivalent. Arrays use the element type plus array syntax in either shorthand or generic form. Objects render properties as `key: elementType;`. In extracted mode, collect all nested objects first and generate named types for each.
...
- Test with various JSON structures: simple objects, nested objects, arrays of primitives, arrays of objects, mixed types, null values, empty objects/arrays
